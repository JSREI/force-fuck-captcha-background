...
export async function runLocalRestore(
  inputDir: string,
  outputDir: string,
  options: LocalRestoreRunOptions & { recursive?: boolean; maxErrorItems?: number } = {}
  ...
  const maxErrorItems = options.maxErrorItems ?? MAX_ERROR_ITEMS;
  ...
  const pushError = (filePath: string, reason: string) => {
    if (errors.length >= maxErrorItems) {
      return;
    }
